...
class ArrayRobot(VecTask):
    ...
    def __init__(self, cfg):
        ...
        self.goal_pos = torch.tensor(cfg.goal.pos, device=self.data_device)     # (3,)
    ...
    def _set_envs(self, cfg):
        ...
        self.base_unit_pos = torch.tensor(rb_states[1][0][0])   # [3, ]
        next_unit_pos = torch.tensor(rb_states[4][0][0])
    ...
    def _load_assets(self, cfg):
        ...
        self.init_asset_dof_states['pos'][:] = (self.dof_upper_limit - self.dof_lower_limit) / 2 + self.dof_lower_limit
    ...
    def _allocate_buffers(self):
        ...
        self.obs_buf = torch.zeros(self.num_envs, self.obs_dim, device=self.data_device, dtype=torch.float32)
    ...
    def _reset_idx(self, env_ids):
        ...
        # TODO: reset centroid & trans_diff & rot_diff buffer

    def reset(self) -> VecEnvObs:
        # TODO: reset & reset_idx
    ...
    def get_dof_shift_from_action(self, action: int, env_id: int):
        ...
        local_dof_shift = self.dct_handler.idct(diff_freq.reshpe(1, -1))    # [1, dim_local, dim_local]
    ...
    def pad_from_local(self, local_tensor: torch.Tensor, env_id: int):
        ...
        half_dim = int((self.dct_handler.dim_local - 1) / 2)
        global_tensor = torch.nn.functional.pad(local_tensor,
                                                (centroid[0] - half_dim, self.robot_side - centroid[0] - half_dim - 1,
                                                 centroid[1] - half_dim, self.robot_side - centroid[1] - half_dim - 1),
                                                'replicate')
    ...
    def pre_physics_step(self, actions: torch.Tensor):
        ...
        self.dof_position_targets += dof_shifts
        self.dof_position_targets = torch.max(torch.min(self.dof_position_targets, self.dof_upper_limit),
    ...
    def compute_observations(self):
        # object states
        obj_pos = self._get_norm_obj_pos()  # [num_envs, 3]
        ...
        return torch.cat([obj_pos, obj_ori, obj_diff_pos, robot_dct], dim=-1)
    ...
    def select_from_global(self, global_tensors: torch.Tensor):
        ...
        for env_id, global_tensor in enumerate(global_tensors):
            ...
            local_tensors.append(global_tensor[x - half_dim:x + half_dim + 1, y - half_dim:y + half_dim + 1])
    ...
    def step(self, actions: np.ndarray):
        # TODO: pre-physics, simulate, fetch, render, post-physics
...
def main(cfg):
    ...
    obs = envs.reset()
    for _ in range(200):
        obs, reward, done, info = envs.step(envs.action_space.sample, device="cuda:0")
