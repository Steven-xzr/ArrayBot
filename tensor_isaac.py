...
for i in range(num_envs):
    ...
    # add actor
        ...
        pose_robot.p = gymapi.Vec3(0.02 * j, 0, -0.02)
...
rb_states = gym.get_actor_rigid_body_states(envs[0], 0, gymapi.STATE_POS)


gym.prepare_sim(sim)
#

all_actor_indices = torch.arange(num_envs * (robot_side + 1), dtype=torch.int32).view(num_envs, robot_side + 1)
...
vec_root_tensor = gymtorch.wrap_tensor(root_tensor).view(num_envs, -1, 13)
root_states = vec_root_tensor
object_positions = vec_root_tensor[..., -1, 0:3]

vec_dof_tensor = gymtorch.wrap_tensor(dof_state_tensor).view(num_envs, robot_side, robot_side, 2)
dof_states = vec_dof_tensor

gym.refresh_actor_root_state_tensor(sim)

init_root_states = root_states.clone()

# root_states[[0, 3]] = init_root_states[[0, 3]] + 0.1
# actor_indices = all_actor_indices[[0, 3]].flatten()
...
global_index = gym.get_actor_dof_index(envs[0], robot_handles[0], 0, gymapi.DOMAIN_SIM)
#
...
# for _ in tqdm(range(1000)):
    ...
    if counter % 2 == 0:
        target = torch.rand(total_dof) * (upper_limit - lower_limit) + lower_limit
